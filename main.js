...
const cartaDoblada = document.getElementById('carta-doblada');
const textoCarta = document.getElementById('texto-carta');
const mensajeInicial = document.getElementById('mensaje-inicial');

const mensaje = `Mi niña hermosa,

Solo quería recordarte lo especial que eres para mí.
Cada día a tu lado es un regalo y mi corazón late más fuerte por ti.
Ya se que estaremos mas lejos, pero quiero que sepas que siempre estaré contigo.
Y estoy tan feliz de estar contigo y de la gran mujer que eres.
...
cartaDoblada.addEventListener('click', () => {
  if (cartaDoblada.classList.contains('abierta') || escribiendo) return;
  cartaDoblada.classList.add('abierta');
  ...
  setTimeout(() => {
    ...
    setTimeout(() => {
      ...
      contenido.style.minHeight = textoCarta.scrollHeight + 48 + 'px';
    }, 1000);
  }, 1200);
});
...
function escribirCarta(texto, elemento, i) {
  ...
  if (i <= texto.length) {
    ...
    setTimeout(() => escribirCarta(texto, elemento, i + 1), 18);
  } else {
    escribiendo = false;
  }
}